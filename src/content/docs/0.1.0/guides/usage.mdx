...
import schemaImage from '../../../../assets/nf-mouse-schema.png';

<div class="centered">
  <img src={schemaImage.src} alt="nf-mouse pipeline schema" />
</div>

:::note[Parameters and profiles]
A pipeline like `nf-mouse` is highly flexible and configurable. To simplify usage for anyone who wants to run it 
...
<Steps>
  1. **`pre_qc`:**
     ...
     :::caution
     **If you want to use the output of the **pre QC** data you must add --use_preqc. See [parameters section](/nf-mouse/0.1.0/guides/parameters).**
     :::
...
<Tabs>
  <TabItem label="Command">
    ```bash
    nextflow run scilus/nf-mouse -r 0.1.0 \
...
<Steps>
  1. **`--input`**: for the path to your BIDS directory
     ...
     For more details on how to organize your input folder, please refer to the [inputs section](/nf-mouse/0.1.0/guides/inputs).
  ...
  3. **`-r`**: Release nf-mouse
      ...
      You need to select the `nf-mouse`release you want to use. [Release available](https://github.com/scilus/nf-mouse/releases).
...
:::note
More parameters can be tuned to your own usage, for a concise list of the most common parameters, you can run `nextflow run scilus/nf-mouse -r main --help`. Otherwise, for the complete list of parameters, please refer to the [parameters section](/nf-mouse/0.1.0/guides/parameters) of the documentation.
:::

:::caution[Running the pipeline in offline mode]
If the computer nodes you are planning to use do not have access to the internet, there is a few steps to go through to make sure the containers and templates are available during pipeline execution. Please head over to [running with no internet access section](/nf-mouse/0.1.0/guides/nointernet).
:::
